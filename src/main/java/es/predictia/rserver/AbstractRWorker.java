package es.predictia.rserver;

import java.util.concurrent.Future;
import java.util.concurrent.atomic.AtomicBoolean;

/** Runs R code and then closes the session
 * @author Max
 *
 */
public abstract class AbstractRWorker implements RWorker {

	private AtomicBoolean isStarted = new AtomicBoolean(false),
			isFinished = new AtomicBoolean(false),
			anyErrors = new AtomicBoolean(false);
	
	protected abstract void runWithinSession(Rsession session) throws Throwable;
	
	@Override
	public void run(Rsession session) throws Throwable {
		this.isStarted.set(true);
		try{
			runWithinSession(session);
		}catch(Throwable e){
			this.anyErrors.set(true);
			throw e;
		}finally{
			session.close();
			this.isFinished.set(true);
		}
	}

	public boolean isStarted() {
		return isStarted.get();
	}
	
	public boolean isFinished() {
		return isFinished.get();
	}

	public boolean anyErrors() {
		return anyErrors.get();
	}
	
	/** Submits worker with {@link RSessionRequest} to the {@link RSessionFactory} and waits till it finishes
	 * @param sessionFactory
	 * @param request
	 * @throws Exception if any error occurs 
	 */
	public void runAndWait(RSessionFactory sessionFactory, RSessionRequest request) throws Exception{
		run(sessionFactory, request).get();
	}
	
	/**
	 * Submits worker with {@link RSessionRequest} to the {@link RSessionFactory}
	 * @param sessionFactory
	 * @param request
	 * @throws Exception if any error occurs
	 */
	public Future<RWorker> run(RSessionFactory sessionFactory, RSessionRequest request) throws Exception{
		return RWorkers.runWithinRsession(this, sessionFactory, request);
	}
	
}
